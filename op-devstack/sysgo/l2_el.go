...
func L2ELWithSupervisor(supervisorID stack.SupervisorID) L2ELOption {
	return L2ELOptionFn(func(p devtest.Scope, id stack.L2ELNodeID, cfg *L2ELConfig) {
...
type L2ELOption interface {
	Apply(p devtest.Scope, id stack.L2ELNodeID, cfg *L2ELConfig)
}
...
type L2ELOptionFn func(p devtest.Scope, id stack.L2ELNodeID, cfg *L2ELConfig)

var _ L2ELOption = L2ELOptionFn(nil)

func (fn L2ELOptionFn) Apply(p devtest.Scope, id stack.L2ELNodeID, cfg *L2ELConfig) {
	fn(p, id, cfg)
}
...
func (l L2ELOptionBundle) Apply(p devtest.Scope, id stack.L2ELNodeID, cfg *L2ELConfig) {
	for _, opt := range l {
