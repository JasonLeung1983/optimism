...
func (cfg *OPRBuilderNodeConfig) LaunchSpec(p devtest.Scope) (args []string, env []string) {
...
type OPRBuilderNodeOption interface {
	Apply(p devtest.Scope, id stack.OPRBuilderNodeID, cfg *OPRBuilderNodeConfig)
}

type OPRBuilderNodeOptionFn func(p devtest.Scope, id stack.OPRBuilderNodeID, cfg *OPRBuilderNodeConfig)

var _ OPRBuilderNodeOption = OPRBuilderNodeOptionFn(nil)

func (fn OPRBuilderNodeOptionFn) Apply(p devtest.Scope, id stack.OPRBuilderNodeID, cfg *OPRBuilderNodeConfig) {
	fn(p, id, cfg)
}
...
func (b OPRBuilderNodeOptionBundle) Apply(p devtest.Scope, id stack.OPRBuilderNodeID, cfg *OPRBuilderNodeConfig) {
	for _, opt := range b {
...
func OPRBuilderWithP2PConfig(addr string, port int, nodeKeyHex string, staticPeers, trustedPeers []string) OPRBuilderNodeOption {
	return OPRBuilderNodeOptionFn(func(p devtest.Scope, id stack.OPRBuilderNodeID, cfg *OPRBuilderNodeConfig) {
		cfg.P2PAddr = addr
		cfg.P2PPort = port
...
func (b *OPRBuilderNode) Start() {
	...
	stdOut := logpipe.LogProcessor(func(line []byte) {
		logpipe.ToLogger(b.logger.New("component", "op-OPRbuilderNode", "src", "stdout"))(logpipe.ParseRustStructuredLogs(line))
	})
	stdErr := logpipe.LogProcessor(func(line []byte) {
		logpipe.ToLogger(b.logger.New("component", "op-OPRbuilderNode", "src", "stderr"))(logpipe.ParseRustStructuredLogs(line))
	})
