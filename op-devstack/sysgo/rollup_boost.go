...
func (r *RollupBoostNode) Start() {
	...
	stdOut := logpipe.LogProcessor(func(line []byte) {
		...
		attrs := append(e.LogFields(), slog.String("stream", "stdout"))
		r.logger.Log(e.LogLevel(), e.LogMessage(), attrs...)
	})
	stdErr := logpipe.LogProcessor(func(line []byte) {
		e := logpipe.ParseRustStructuredLogs(line)
		attrs := append(e.LogFields(), slog.String("stream", "stderr"))
		r.logger.Log(e.LogLevel(), e.LogMessage(), attrs...)
	})
	...
	rpcUpstreamURL := "http://" + cfg.RPCHost + ":" + strconv.Itoa(cfg.RPCPort)
	...
	return stack.AfterDeploy(func(orch *Orchestrator) {
		...
		for _, opt := range opts {
			if linkOpt, ok := opt.(interface {
				applyNode(p devtest.Scope, id stack.RollupBoostNodeID, r *RollupBoostNode)
			}); ok {
...
func (cfg *RollupBoostConfig) LaunchSpec(p devtest.Scope) (args []string, env []string) {
	...
	args = append(args, "--rpc-host="+cfg.RPCHost, "--rpc-port="+strconv.Itoa(cfg.RPCPort))
