...
func TestFlashblocksTransfer(gt *testing.T) {
	if os.Getenv("ROLLUP_BOOST_EXEC_PATH") == "" ||
		...
		gt.Skip("skipping flashblocks transfer test: required exec path env vars not set")
	}
	...
	for name, opt := range matrix {
		...
		gt.Run(name, func(gt *testing.T) {
			...
			presets.RunWithSysgoOrchestrator(t, func(orch stack.Orchestrator) {
				...
				runFlashblocksTransfer(t, sys, name)
			})
		})
	}
}

func runFlashblocksTransfer(t devtest.T, sys *presets.SingleChainWithFlashblocks, backend string) {
	logger := t.Logger().With("backend", backend)
	tracer := t.Tracer()
	ctx := t.Ctx()
